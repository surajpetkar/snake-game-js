...
function drawFood(){
    ...
    // ctx.arc(foodX, foodY, unitSize/2.5, 0, 2*Math.PI);
    // ctx.fill();
};
function moveSnake(){
...
function drawSnake(){
    ...
    snake.forEach(snakePart => {
        // ctx.beginPath();
        // ctx.arc(snakePart.x, snakePart.y, unitSize/2.5, 0, 2*Math.PI);
        // ctx.fill();
...
function changeDirection(event){
    const keyPressed = event?.keyCode ? event.keyCode : event;
    const LEFT = 37;
    const UP = 38;
    const RIGHT = 39;
    const DOWN = 40;
